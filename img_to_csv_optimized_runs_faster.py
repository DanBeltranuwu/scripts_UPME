...
bands=[]
band_names=[]
for i,band in enumerate(static_bands):
    matrix=np.matrix(band)
    band_name=names[i].split('_')[0]
...
frames=[]
for i,ban in enumerate(classes_arr):
    matrix=np.matrix(ban)
    clas_arr=np.array(matrix.flatten())[0]
    to_append=df[clas_arr!=no_data]
    to_append['clase']=classes[i]
    frames.append(to_append)
print('finished frames')
band_names.append('clase')
...
#     colu=name.split('_')[0]
#     matrix=np.matrix(static_bands[i])
#     df[colu]=np.array(matrix.flaten())[0]
df.drop_duplicates(subset=df.columns.difference(['clase']))
print(df.head())
print(df.shape)
df.to_csv(text_prefix+'data'+text_suffix)
